...
class TECH(param.Parameterized):
    ...
    def impact_hours_view(self):
        ...
        df['Passes Minimum'] = df['Total XDAI Raised'] >= self.min_max_raise[0] - 2
        feasible_raise = self.hatch_oracle_ratio * self.total_cstk_tokens
        df['Feasible Raise'] = df['Passes Minimum'] & (df['Total XDAI Raised'] <= feasible_raise + 2)
        ...
        impact_hours_plot_feasible = df[df['Feasible Raise']==True].hvplot.area(color='blue', y=['Impact Hour Rate'], title='Impact Hour Rate', x='Total XDAI Raised',  xformatter='%.0f', hover=True)
        impact_hours_plot = impact_hours_plot_passes * impact_hours_plot_feasible * impact_hours_plot_fails
        target_plot = hv.VLine(self.target_raise).opts(color='#E31212') * hv.HLine(target_impact_hour_rate).opts(color='#E31212')
